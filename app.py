...
# Main Title
st.title("QuLab: Stochastic calculus")
st.divider()
...
if st.session_state.current_page == '1. Standard Brownian Motion (W_t)':
    ...
    st.markdown(r"A stochastic process $X_t$ is a martingale with respect to a filtration $\mathcal{F}_t$ and probability measure $P$ if it satisfies three conditions:")
    st.markdown(r"1.  $$E[|X_t|] < \infty$$ for all $$t \geq 0$$ (integrability).")
    st.markdown(r"where $E[|X_t|]$ is the expected absolute value of $X_t$.")
    st.markdown(r"2.  $X_t$ is $\mathcal{F}_t$-adapted (knowable at time $t$).")
    st.markdown(r"where $\mathcal{F}_t$ represents the information available up to time $t$.")
    st.markdown(r"3.  For any $t, s \geq 0$, the conditional expectation satisfies:")
    ...
    st.markdown(r"This means the best prediction of a future value of the process, given current information, is simply its current value. For $W_t$, we use the property that $W_{t+s} - W_t$ is independent of $\mathcal{F}_t$ and has an expectation of 0.")
    ...
    st.markdown(r"Aisha observes that while individual paths of $W_t$ wander randomly, the average of a large number of simulated paths remains centered around zero (its initial value). This visually confirms the martingale property for $W_t$: the expected future value, given the current information, is indeed the current value (which is 0 for $W_t$ starting at $W_0=0$). This verification reinforces her understanding of its fundamental nature in arbitrage-free modeling.")

# --- PAGE: 2. Squared Brownian Motion Minus Time (W_t^2 - t) ---
...
# --- PAGE: 4. Risk-Neutral Valuation & FTAP ---
if st.session_state.current_page == '4. Risk-Neutral Valuation & FTAP':
    ...
    st.markdown(r"Aisha's core responsibility includes ensuring the models used for pricing options and other derivatives are robust and arbitrage-free. The martingale property is central to this. Under a risk-neutral probability measure $\mathbb{Q}$, the *discounted* price of any tradable asset (or any derivative thereon) must be a martingale. If this were not true, arbitrage opportunities would exist, which contradicts efficient market hypotheses.")
    ...
    st.markdown(r"For arbitrage-free markets, under the risk-neutral measure $\mathbb{Q}$, this discounted process must be a martingale:")
