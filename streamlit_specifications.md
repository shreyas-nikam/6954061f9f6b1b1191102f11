...
# This wrapper handles matplotlib plots generated by source.py functions.
# It's assumed that `plt.show()` calls within source.py are managed by the
# Streamlit environment such that `st.pyplot(plt.gcf())` successfully captures
# and displays the generated figure.
...
# Update session state and trigger rerun if page changed
if page_selection != st.session_state.current_page:
    st.session_state.current_page = page_selection
    st.rerun()

# --- General Simulation Parameters Sidebar (conditional rendering) ---
...
# --- Main Content Area (conditional rendering based on current_page) ---
